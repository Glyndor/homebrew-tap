# typed: false
# frozen_string_literal: true

# Generated by scripts/render-formulae.sh from the signed release of Glyndor/podup.
# Do not edit by hand — edit the generator and re-run the update workflow.
class Podup < Formula
  desc "Docker-compose translator and runner for rootless Podman"
  homepage "https://github.com/Glyndor/podup"
  version "3.7.1"
  license "MIT"

  on_macos do
    on_arm do
      url "https://github.com/Glyndor/podup/releases/download/v3.7.1/podup-darwin-arm64"
      sha256 "cf34d8253088f49ca894712765315b9b9a6e45c505417800a0b5afa83d2a1005"
    end
    on_intel do
      url "https://github.com/Glyndor/podup/releases/download/v3.7.1/podup-darwin-x86_64"
      sha256 "e527dfd416c457f58540a49efea90b67e481bb4080a8c30a21ce34b854c1722b"
    end
  end

  def install
    # A bare-binary download stages under its release-asset name. Take that name
    # from the generator's table, which is the same source the urls above come
    # from, rather than globbing for one: a product whose assets are not named
    # "<tool>-darwin-<arch>" would match nothing and install an empty formula.
    asset = Hardware::CPU.arm? ? "podup-darwin-arm64" : "podup-darwin-x86_64"
    bin.install asset => "podup"
  end

  test do
    # The argument comes from the generator's table: brew audit --strict
    # requires a test block, and not every product answers --version.
    system "#{bin}/podup", "--version"
  end
end
